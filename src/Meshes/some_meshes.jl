...
function circmesh(center, rad, h)
    cx, cy = center
    n = Int(1 + 2π * rad ÷ h)
    θ = range(start = 0.0, stop = 2π, length = n)[1:(end - 1)]
    verts = [cx .+ cos.(θ');cy .+ sin.(θ')]
    return hpmesh(verts, h)
end
circmesh(rad, h) = circmesh((0.0, 0.0), rad, h)
circmesh(h) = circmesh(1.0, h)
...
squaremesh(a, h) = rectmesh(0.0, a, 0.0, a, h)
