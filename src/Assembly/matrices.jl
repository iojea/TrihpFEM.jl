...
function collapse(factors,aff::AffineToRef,tensors)
    ...
    out = sum(_reduce(f,t) for (f,t) in zip(fac,tensors))
    out
end
...
function _reduce(f::Tensor{2,2},t::Tensor{2,2})
    f⊡t
end
function _reduce(f,t)
    f⋅t
end

function variablefactors(f,aff,sch::Quadrature{D,F,P}) where {D,F,P}
    s = size(f(zeros(F,D)))
    dim = length(s)
    ord = dim > 0 ? 2 : 1
    if dim > 0
...
function ref_tensor(inte::Integrand{ConstantCoeff, T, Order{B}, 2}, degs) where {T, B}
    (; factor, funs) = inte
    b₁ = basis(funs[1], degs); b₂ = basis(funs[2], degs)
    o₁, o₂ = operator.(funs)
    f = tensorize(factor)
    return (outer(_tensor(o₁(φ), o₂(ψ), Val(sum(B))),f) for φ in b₁, ψ in b₂)
end
...
function ref_tensor(inte::Integrand{ConstantCoeff,T,Order{B},1},degs) where {T,B}
    (;factor,funs) = inte
    b = basis(funs[1],degs)
    o = operator(funs[1])
    f = tensorize(factor)
    return (outer(_tensor(o(φ),Val(sum(B))),f) for φ in b)
end
...
i2m(i,n) = CartesianIndex(mod1(i,n),1+(i-1)÷n) 
function add_to_matrix_naive!(ivec, jvec, vals, t::Term{ConstantCoeff, O, T, 2, M}) where {O, T, M}
    (; integrand, measure) = t
    (; mesh,aux) = measure
    ...
    for el in Measures.elements(measure)
        ...
        f = tensorize(factor)
        aff = AffineToRef(mesh.points[el])
        for (i,φ) in enumerate(b₁), (j,ψ) in enumerate(b₂)
            v[i,j] = _integrate(φ,o₁,ψ,o₂,f,aff)
        end
        locdof = dof(el,mesh)
        dim = length(locdof)
        C = aux[degs].C
        ...
        ivec[r:(r + dim^2 - 1)] .= repeat(locdof, dim)
        jvec[r:(r + dim^2 - 1)] .= repeat(locdof, inner = dim)
        copyto!(vals[r:(r + dim^2 - 1)],v[1:dim,1:dim])
        r += dim^2
    end
end
"""
...
function add_to_rhs!(vals, t::Term{ConstantCoeff, O, T, 1, M}) where {O, T, M}
    ...
    v = zeros(floattype(mesh),nmax)
        ...
        dim = length(locdof)
        C = aux[degs].C
        copyto!(v[1:dim],(collapse(aff, loctensor[i]) for i in 1:dim))
        copyto!(v,jac(aff) * C' * view(v,1:dim))
        copyto!(vals[locdof],v[1:dim])
        r += dim
    end
end
