"""
  collapser(::Order,aff::AffineTransformation)
...
function buildmatrix!(::Constant,ord::Order{B},ivec,jvec,vals,fun,measure,space) where B
    ...
    for tri in keys(trilist)
        ...
        if isin
            ...
            loctensor = build_local_tensor(Constant(),ord,fun,base(p))
...
function build_local_tensor(::Constant,::Order{B},fun,base) where B
    ...
    ind = CartesianIndices(size(base))
    for (k,φ) in zip(ind,base)
        local_tensor[k,..] .= ref_integrate(fun(φ...))
    end
    return local_tensor
end
